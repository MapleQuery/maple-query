...
Additive SSE event types emitted by v2 (v1 consumers ignore unknown types): `phase_start`, `turn_record` (live now), `triage_result` (live now), `reformulation` (live now), `verification`, `plan_hint` (declared for the phase implementations to fill). Prompt v2 (`agent/prompts/v2/system.j2`, ≤1.3K tokens CI-enforced by `tests/unit/test_prompt_v2.py`) drops every rule the tools now enforce and adds the citation contract: datasets are cited as `[<title>](/datasets/<package_id>)` with both parts verbatim from the turn's tool results.
