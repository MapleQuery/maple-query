...
Modes (`WHENRICH_AGENT_TRIAGE_MODE`): `off` — kill switch, dispatch wires `PassthroughTriage`; `log` — shadow mode (default), classify + emit `triage_result {category, confidence, elapsed_ms, enforced: false}` but always continue (a permanent mode for re-qualifying taxonomy changes on live traffic, not a rollout hack); `act` — short-circuit confidently-classified non-in_scope turns. Every turn emits `triage_result` on the SSE stream, the turn span carries `metadata.triage`, and the v2 `turn_record` carries `triage_category`.
