...
- **Loop selection** — `WHENRICH_AGENT_LOOP_IMPL` (`v1` default | `v2`) picks which orchestrator serves `/chat`: the v1 loop or the v2 phase pipeline (see the semantic-enrich doc, "Turn pipeline v2"). Both ship in one image; flipping is a redeploy with one env var. Startup logs `loop_impl_selected`; `deps.build_app_state` builds the matching deps flavour via `semantic_enrich.core.agent_dispatch.build_loop_handle`, and routes drive turns through `AppState.run_turn` without importing either loop module. The `/chat` body accepts an optional `turn_records` list (currently ignored by both loops); v2 additionally streams `phase_start` / `turn_record` SSE events, which the web app's handler already skips as unknown types.
