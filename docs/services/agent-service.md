...
- **Loop selection** — `WHENRICH_AGENT_LOOP_IMPL` (`v2` default post-cutover | `v1`) picks which orchestrator serves `/chat`: the v2 phase pipeline or the legacy v1 loop (see the semantic-enrich doc, "Turn pipeline v2"). Both ship in one image; flipping is a redeploy with one env var. Startup logs `loop_impl_selected`; `deps.build_app_state` builds the matching deps flavour via `semantic_enrich.core.agent_dispatch.build_loop_handle`, and routes drive turns through `AppState.run_turn` without importing either loop module.
- **`turn_records` request field** — the `/chat` body accepts an optional `turn_records` list: the client-held memory records the v2 loop emitted on prior turns (`turn_record` SSE event, one per turn, just before `done`). The web app stores them in localStorage and echoes them back; the server validates defensively (invalid records dropped with a log, never a 400) and stays stateless. v1 ignores the field.
- **v2 SSE event types** (additive; unknown types are skipped by v1-era consumers): `phase_start {phase}`, `triage_result {category, confidence, elapsed_ms, enforced}`, `reformulation {original_query, reformulated_query, top_similarity_before}`, `verification {fits, action, confidence, reason, enforced}`, `plan_hint {records_used}`, `cache_hit {cache_key_prefix}`, `turn_record {record}`.
- **Rollback procedure (cutover soak)** — any correctness incident: redeploy with `WHENRICH_AGENT_LOOP_IMPL=v1` (minutes; v1 code remains until the post-soak deletion). Triage or verify misbehaving alone: demote just that phase with `WHENRICH_AGENT_TRIAGE_MODE=log` / `WHENRICH_AGENT_VERIFY_MODE=log` without leaving v2. A rollback halts the v1 deletion until root-caused.
