...
`requests-mock` adapter to stub CKAN's `package_search` endpoint. The
script lives outside `src/` so it is imported via an explicit
sys.path insertion (matches how the script wires itself in
production).
"""
...
def _mock_package_search(
    m: requests_mock.Mocker,
    *,
    pages: list[list[dict[str, Any]]],
) -> None:
    """Mock /package_search to return each page in order.

    Each page is a list of dataset dicts (each with `id` and `resources`).
    `count` is set to the total dataset count across all pages so the
    walker terminates on the last page.
    """
    ...
    responses = [
        {
            "json": {
                "success": True,
                "result": {"count": total, "results": page},
            },
        }
        for page in pages
    ]
    # Tail page so an over-fetch (loop bug) still terminates cleanly.
    responses.append(
        {"json": {"success": True, "result": {"count": total, "results": []}}},
    )
    m.get(f"{_CKAN_BASE}/package_search", responses)
...
def test_backfill_misses_unknown_urls() -> None:
    ...
    with requests_mock.Mocker() as m:
        _mock_package_search(
            m,
            pages=[[{"id": "pkg-a", "resources": [{"url": "https://ckan.test/a.csv"}]}]],
        )
...
def test_backfill_paginates_across_pages() -> None:
    """The walker consumes every page until the result list is empty."""
    bq = FakeBqClient()
    _seed_query_result(bq, rows=[])
    ...
    with requests_mock.Mocker() as m:
        _mock_package_search(
            m,
            pages=[
                [{"id": "pkg-a", "resources": [{"url": "https://ckan.test/a.csv"}]}],
                [{"id": "pkg-b", "resources": [{"url": "https://ckan.test/b.csv"}]}],
                [{"id": "pkg-c", "resources": [{"url": "https://ckan.test/c.csv"}]}],
            ],
        )
    ...
    assert summary.packages_walked == 3
    assert summary.resources_indexed == 3
...
def test_backfill_reset_nulls_then_refetches() -> None:
    ...
    with requests_mock.Mocker() as m:
        _mock_package_search(
            m,
            pages=[[{"id": "pkg-correct", "resources": [{"url": "https://ckan.test/a.csv"}]}]],
        )
                ...
                [
                    {
                        "id": "pkg-a",
                        "resources": [
                            {"url": "/dataset/x/resource/y/download/data.csv"},
                        ],
                    },
                ],
            ],
